...
function AdminUsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!qq) return users;
    return users.filter((u) => (u.fullName || "").toLowerCase().includes(qq) || (u.email || "").toLowerCase().includes(qq));
  }, [q, users]);
  ...
  if (!hasRole("admin")) {
    return (
      <PageContainer>
        ...
        <Card className="p-4">
          <p className="text-sm text-muted-foreground">Você precisa ser admin para gerenciar permissões.</p>
        </Card>
      </PageContainer>
    );
  }
                ...
                <TableRow key={u.id}>
                  ...
                  <TableCell className="space-x-1">
                    {(u.roles.length ? u.roles : (["viewer"] as AppRole[])).map((r) => (
                      <Badge key={`${u.id}-${r}`} variant={r === "admin" ? "default" : "outline"} className="uppercase text-[10px]">
                        {r}
                      </Badge>
                    ))}
                  </TableCell>
